...
| 6 | Ordem no `cadastro-submit`: salvar → grupo (como hoje) → **contrato** → **Conta Azul** → aviso no Staff. Contrato e cobrança rodam em background (`void`), o cliente não espera. **São dois avisos, de propósito:** o do grupo sai na hora e o de contrato + cobrança sai quando fica pronto — juntar os dois atrasaria o aviso do grupo pelo tempo da leitura por IA e da chamada ao Conta Azul. | Resposta rápida ao cliente; Staff sabe do grupo sem esperar. |
